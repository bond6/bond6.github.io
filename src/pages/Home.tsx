      ...
      <IonAlert
        ...
        <div className="hero-card">
          ...
          <div className="hero-icons">
            <img className="hero-icon-sm" alt="Facebook" src="/assets/img/facebook.png" onClick={function() {window.open("https://www.facebook.com/ICAInternationalChemicals/?rf=1585922241735402",'_blank');}}/>
            <img className="hero-icon-web" alt="Website" src="/assets/img/website_logo.png" onClick={function() {window.open("http://icaonline.co.za",'_blank');}}/>
            <img className="hero-icon-mag" alt="Magazine" src="/assets/img/bros.png" onClick={function() {window.open("https://visualprojects.co.za/Info-Hub/ICA/",'_blank');}} />
            <img className="hero-icon-mag" alt="Products" src="/assets/img/bros1.png" onClick={function() {window.open("https://testicamobilemagazine.web.app/products_bros.html",'_blank');}} />
          </div>
      ...
      <IonFooter>
        <IonToolbar color="primary" style={{textAlign: 'center', padding: '4px 8px'}}>
          ...
          <IonButton color="light" onClick={async function() {
            ...
            // Build results data for image
            var cards = demoEl.querySelectorAll('.mrl-card');
            var results: {name: string; mrl: string; products: string}[] = [];
            cards.forEach(function(card) {
              var n = card.querySelector('.mrl-card-name');
              var m = card.querySelector('.mrl-badge');
              var p = card.querySelector('.mrl-card-plist');
              results.push({
                name: n ? (n.textContent || '') : '',
                mrl: m ? (m.textContent || '') : '',
                products: p ? (p.textContent || '') : ''
              });
            });

            // Generate branded image on canvas
            var rowH = 52;
            var headerH = 140;
            var footerH = 50;
            var w = 700;
            var h = headerH + (results.length * rowH) + 40 + footerH;
            ...
            // Header bar
            ctx.fillStyle = '#00a6b6';
            ctx.fillRect(0, 0, w, headerH);

            // Header text
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 28px -apple-system, Helvetica, Arial';
            ctx.fillText('ICA Citrus MRL V6.0', 24, 40);

            ctx.font = '20px -apple-system, Helvetica, Arial';
            ctx.fillText('Fruit: ' + fruitName, 24, 75);
            ctx.fillText('Countries: ' + countryNames.join(', '), 24, 105);

            ctx.font = '14px -apple-system, Helvetica, Arial';
            ctx.fillStyle = 'rgba(255,255,255,0.7)';
            ctx.fillText(results.length + ' results', 24, 130);

            // Table header
            var tableY = headerH + 16;
            ctx.fillStyle = '#007aff';
            ctx.fillRect(16, tableY, w - 32, 36);
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 14px -apple-system, Helvetica, Arial';
            ctx.fillText('ACTIVE INGREDIENT', 28, tableY + 24);
            ctx.fillText('MRL', 340, tableY + 24);
            ctx.fillText('PRODUCTS', 420, tableY + 24);
            tableY += 36;

            // Table rows
            results.forEach(function(r, i) {
              ctx.fillStyle = i % 2 === 0 ? '#ffffff' : '#f8f8fa';
              ctx.fillRect(16, tableY, w - 32, rowH);

              ctx.fillStyle = '#1c1c1e';
              ctx.font = '600 15px -apple-system, Helvetica, Arial';
              ctx.fillText(r.name, 28, tableY + 22);

              // MRL badge
              var mrlColor = '#007aff';
              if (r.mrl === '\u221E' || r.mrl === '∞') mrlColor = '#af52de';
              else if (r.mrl === '#') mrlColor = '#af52de';
              else if (parseFloat(r.mrl) <= 2) mrlColor = '#ff9500';
              ctx.fillStyle = mrlColor;
              // Draw rounded rect badge
              var bx = 335, by = tableY + 6, bw = 50, bh = 26, br = 8;
              ctx.beginPath();
              ctx.moveTo(bx + br, by);
              ...
              ctx.fill();
              ctx.fillStyle = '#fff';
              ctx.font = 'bold 15px -apple-system, Helvetica, Arial';
              ctx.textAlign = 'center';
              ctx.fillText(r.mrl, 360, tableY + 24);
              ctx.textAlign = 'left';

              // Products (truncate if too long)
              ctx.fillStyle = '#636366';
              ctx.font = '13px -apple-system, Helvetica, Arial';
              var prodText = r.products.length > 35 ? r.products.substring(0, 35) + '...' : r.products;
              ctx.fillText(prodText, 420, tableY + 18);
              if (r.products.length > 35) {
                ctx.fillText(r.products.substring(35, 70) + (r.products.length > 70 ? '...' : ''), 420, tableY + 36);
              }
            ...
            ctx.font = '13px -apple-system, Helvetica, Arial';
            ctx.fillText('Generated by ICA Citrus MRL App  •  bond6.github.io', 16, h - 16);

            // Convert to base64 and save as file
            try {
              var base64 = canvas.toDataURL('image/png').split(',')[1];
              var fileName = 'CitrusMRL_' + fruitName.replace(/\s/g, '') + '.png';
              var saved = await Filesystem.writeFile({
                path: fileName,
                data: base64,
              ...
              await Share.share({
                title: 'ICA Citrus MRL - ' + fruitName,
                text: 'MRL results for ' + fruitName + ' (' + countryNames.join(', ') + ')\n\nView in app: https://bond6.github.io',
                url: saved.uri,
                dialogTitle: 'Share MRL Results'
              });
            } catch(e) {
              // Fallback to text only
              await Share.share({
                title: 'ICA Citrus MRL - ' + fruitName,
                text: 'MRL results for ' + fruitName + ' (' + countryNames.join(', ') + ')\n\nView: https://bond6.github.io',
                dialogTitle: 'Share MRL Results'
              });
            }
          }}>
